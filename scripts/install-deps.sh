#!/usr/bin/env bash
# Create venv (if needed), install PyTorch (CUDA) and Python deps for Lira.
set -euo pipefail
...
if ! python -c "import torch" 2>/dev/null; then
  ...
  echo "  (reinstall: remove torch from venv or use a fresh venv; override: LIRA_TORCH_CUDA=cu128)"
fi
...
if [[ -n "${LIRA_SKIP_LLAMA:-}" ]]; then
  echo "LIRA_SKIP_LLAMA=1 — skipping llama-cpp-python"
else
  if python -c "import llama_cpp" 2>/dev/null; then
    echo "llama-cpp-python already installed: $(python -c 'import llama_cpp; print(getattr(llama_cpp, "__version__", "?"))')"
  else
...
ensure_sd_cuda() {
  if [[ "${LIRA_INSTALL_SD_CPU:-}" == "1" ]]; then
    echo "LIRA_INSTALL_SD_CPU=1 — stable-diffusion-cpp stays CPU-only."
    return 0
  fi
  ...
  if sd_cuda_linked 2>/dev/null; then
    echo "stable-diffusion-cpp: CUDA build already present."
    return 0
  fi
  local torch_tag
  torch_tag="$("$ROOT/scripts/pytorch_cuda_index.sh")"
  if [[ "$torch_tag" == "cpu" ]]; then
    echo "stable-diffusion-cpp: CPU-only (no NVIDIA/CUDA for PyTorch)."
    return 0
  fi
  echo "stable-diffusion-cpp: pip wheel lacks CUDA — rebuilding with SD_CUDA=ON…"
  CMAKE_ARGS="-DSD_CUDA=ON" pip install --force-reinstall --no-cache-dir stable-diffusion-cpp-python
  if ! sd_cuda_linked; then
    echo "ERROR: stable-diffusion-cpp built without CUDA. See docs/image-generation.md" >&2
    return 1
  fi
...
ensure_diffusers() {
  if [[ "${LIRA_INSTALL_QWEN_CPU:-}" == "1" ]]; then
    echo "LIRA_INSTALL_QWEN_CPU=1 — skipping diffusers (Qwen Image Edit)."
    return 0
  fi
  if python -c "from diffusers import QwenImageEditPlusPipeline" 2>/dev/null; then
    echo "diffusers (Qwen Image Edit): already installed."
    return 0
  fi
  local torch_tag
  torch_tag="$("$ROOT/scripts/pytorch_cuda_index.sh")"
  if [[ "$torch_tag" == "cpu" && "${LIRA_INSTALL_OPTIONAL:-}" != "1" ]]; then
    echo "diffusers: skipped (PyTorch CPU). For image-edit slot: LIRA_INSTALL_OPTIONAL=1"
    return 0
  fi
  echo "Installing diffusers (Qwen Image Edit)…"
  pip install -r "$ROOT/requirements-optional.txt"
  python -c "from diffusers import QwenImageEditPlusPipeline; print('diffusers ok')"
}
...
echo ""
echo "Done. Running import smoke test:"
